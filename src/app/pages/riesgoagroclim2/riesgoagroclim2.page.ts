...
import { previous, iconmarker, leyendariesgo, urlMapaLealeft,
  urlIDESEPDepart,urlIDESEPProv ,urlIDESEPriesgo2} from '../../globales';
import { WmssenamhiService } from '../../services/wmssenamhi.service';

@Component({
  selector: 'app-riesgoagroclim2',
  templateUrl: './riesgoagroclim2.page.html',
  styleUrls: ['./riesgoagroclim2.page.scss'],
})
export class Riesgoagroclim2Page  {
  ...
  previo=previous;
  titulo="Riesgo Agroclimático - Maíz";
  ...
  async ionViewDidEnter() {
    ...
    const riesgomaiz = tileLayer.wms(urlIDESEPriesgo2, {
    ...
    const overlays = {
      ...
      RiesgoMaiz: riesgomaiz
    };

   await this.geolocation.getCurrentPosition(this.options).then(
      (pos: Geoposition) => {
        
        this.map = new Map('riesgomaiz', {center: [ pos.coords.latitude, pos.coords.longitude ],
          zoom: 6,
          maxZoom: 100,
          minZoom: 6,
          layers: [routesmap, riesgomaiz, departamento, provincia]}
      ...
      this.map.on('click',(e)=>{
        ...
        this.wmsenmRiesgo.getInfoRiesgoClimatologico(2,containerPotin.x ,containerPotin.y,sw.lat,sw.lng,ne.lat,ne.lng, size.x, size.y)
        .subscribe((response)=>{
        ...
        legend.onAdd = () => {
          ...
          let labels = [];
          labels.push("<div>Niveles de Riesgo: Maíz</div>");
