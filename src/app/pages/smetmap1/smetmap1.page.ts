...
import { previous, iconmarker, radiacionUVLeg, urlMapaLealeft,urlIDESEPDepart,
        urlIDESEPProv,urlIDESEPRUV48 } from '../../globales';
import { WmssenamhiService } from '../../services/wmssenamhi.service';


@Component({
  selector: 'app-smetmap1',
  templateUrl: './smetmap1.page.html',
  styleUrls: ['./smetmap1.page.scss'],
})
export class Smetmap1Page {
  ...
  async ionViewDidEnter() {
    ...
    const radiacion48 = tileLayer.wms(urlIDESEPRUV48, {
   ...
   await this.geolocation.getCurrentPosition(this.options).then(
      (pos: Geoposition) => {
        //Agregar mapa
        this.map = new Map('radiacion48', {center: [ pos.coords.latitude, pos.coords.longitude ],
          zoom: 6,
          maxZoom: 100,
          minZoom: 6,
          layers: [routesmap, radiacion48, departamento, provincia]}
        ...
        this.map.on('click',(e)=>{
          ...
          this.wmsenmRUV48.getInfoRUV(1,e.latlng.lat, e.latlng.lng)
          .subscribe((response)=>{
        ...
        this.wmsenmRUV48.getInfoRUV(1,pos.coords.latitude, pos.coords.longitude)
          .subscribe((response)=>{
