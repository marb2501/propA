...
import { previous, iconmarker, leyendariesgo, urlMapaLealeft,
  urlIDESEPDepart,urlIDESEPProv ,urlIDESEPriesgo5} from '../../globales';
import { WmssenamhiService } from '../../services/wmssenamhi.service';

@Component({
  selector: 'app-riesgoagroclim5',
  templateUrl: './riesgoagroclim5.page.html',
  styleUrls: ['./riesgoagroclim5.page.scss'],
})
export class Riesgoagroclim5Page  {
  ...
  previo=previous;
  titulo="Riesgo Agroclimático - Café";
  ...
  async ionViewDidEnter() {
    ...
    const riesgocafe = tileLayer.wms(urlIDESEPriesgo5, {
    ...
    const overlays = {
      ...
      RiesgoCafe: riesgocafe
    };

   await this.geolocation.getCurrentPosition(this.options).then(
      (pos: Geoposition) => {
        
        this.map = new Map('riesgocafe', {center: [ pos.coords.latitude, pos.coords.longitude ],
          zoom: 6,
          maxZoom: 100,
          minZoom: 6,
          layers: [routesmap, riesgocafe, departamento, provincia]}
        ...
        this.map.on('click',(e)=>{
          ...
          this.wmsenmRiesgo.getInfoRiesgoClimatologico(5,containerPotin.x ,containerPotin.y,sw.lat,sw.lng,ne.lat,ne.lng, size.x, size.y)
          .subscribe((response)=>{
        ...
        legend.onAdd = () => {
          ...
          let labels = [];
          labels.push("<div>Niveles de Riesgo: Café</div>");
