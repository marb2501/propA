...
import { previous, iconmarker, leyendariesgo, urlMapaLealeft,
  urlIDESEPDepart,urlIDESEPProv ,urlIDESEPriesgo1 } from '../../globales';
import { WmssenamhiService } from '../../services/wmssenamhi.service';

@Component({
  selector: 'app-riesgoagroclim1',
  templateUrl: './riesgoagroclim1.page.html',
  styleUrls: ['./riesgoagroclim1.page.scss'],
})
export class Riesgoagroclim1Page {
  ...
  constructor(private router: Router, private geolocation: Geolocation,
              public wmsenmRiesgo:WmssenamhiService) { }

  previo=previous;
  titulo="Riesgo Agroclimático - Arroz";
  ...
  async ionViewDidEnter() {
    ...
    const riesgoarroz = tileLayer.wms(urlIDESEPriesgo1, {
    ...
    const overlays = {
      ...
      RiesgoArroz: riesgoarroz
    };

   await this.geolocation.getCurrentPosition(this.options).then(
      (pos: Geoposition) => {
        
        this.map = new Map('riesgoarroz', {center: [ pos.coords.latitude, pos.coords.longitude ],
          zoom: 6,
          maxZoom: 100,
          minZoom: 6,
          layers: [routesmap, riesgoarroz, departamento, provincia]}
        ...
        this.map.on('click',(e)=>{
          ...
          this.wmsenmRiesgo.getInfoRiesgoClimatologico(1,containerPotin.x ,containerPotin.y,sw.lat,sw.lng,ne.lat,ne.lng, size.x, size.y)
          .subscribe((response)=>{
        ...
        legend.onAdd = () => {
          ...
          labels.push("<div>Niveles de Riesgo: Arroz</div>");
