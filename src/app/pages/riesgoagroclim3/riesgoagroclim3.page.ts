...
import { previous, iconmarker, leyendariesgo,urlMapaLealeft,
  urlIDESEPDepart,urlIDESEPProv ,urlIDESEPriesgo3 } from '../../globales';
import { WmssenamhiService } from '../../services/wmssenamhi.service';

@Component({
  selector: 'app-riesgoagroclim3',
  templateUrl: './riesgoagroclim3.page.html',
  styleUrls: ['./riesgoagroclim3.page.scss'],
})
export class Riesgoagroclim3Page {
  ...
  previo=previous;
  titulo="Riesgo Agroclimático - Papa";
  ...
  async ionViewDidEnter() {
    ...
    const riesgopapa = tileLayer.wms(urlIDESEPriesgo3, {
    ...
    const overlays = {
      ...
      RiesgoPapa: riesgopapa
    };

   await this.geolocation.getCurrentPosition(this.options).then(
      (pos: Geoposition) => {
        
        this.map = new Map('riesgopapa', {center: [ pos.coords.latitude, pos.coords.longitude ],
          zoom: 6,
          maxZoom: 100,
          minZoom: 6,
          layers: [routesmap, riesgopapa, departamento, provincia]}
        ...
        this.map.on('click',(e)=>{
          ...
          this.wmsenmRiesgo.getInfoRiesgoClimatologico(3,containerPotin.x ,containerPotin.y,sw.lat,sw.lng,ne.lat,ne.lng, size.x, size.y)
          .subscribe((response)=>{
        ...
        this.map.addLayer(markPoint);
         ...
         legend.onAdd = () => {
           ...
           let labels = [];
           labels.push("<div>Niveles de Riesgo: Papa</div>");
