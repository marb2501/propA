...
import { previous, iconmarker, leyendariesgo,urlMapaLealeft,
  urlIDESEPDepart,urlIDESEPProv ,urlIDESEPriesgo6 } from '../../globales';
import { WmssenamhiService } from '../../services/wmssenamhi.service';

@Component({
  selector: 'app-riesgoagroclim6',
  templateUrl: './riesgoagroclim6.page.html',
  styleUrls: ['./riesgoagroclim6.page.scss'],
})
export class Riesgoagroclim6Page {
  ...
  previo=previous;
  titulo="Riesgo Agroclimático - Pasto";
  ...
  async ionViewDidEnter() {
    ...
    const riesgopasto = tileLayer.wms(urlIDESEPriesgo6, {
    ...
    const overlays = {
      ...
      RiesgoPasto: riesgopasto
    };

   await this.geolocation.getCurrentPosition(this.options).then(
      (pos: Geoposition) => {
        
        this.map = new Map('riesgopasto', {center: [ pos.coords.latitude, pos.coords.longitude ],
          zoom: 6,
          maxZoom: 100,
          minZoom: 6,
          layers: [routesmap, riesgopasto, departamento, provincia]}
        ...
        this.map.on('click',(e)=>{
          ...
          this.wmsenmRiesgo.getInfoRiesgoClimatologico(6,containerPotin.x ,containerPotin.y,sw.lat,sw.lng,ne.lat,ne.lng, size.x, size.y)
          .subscribe((response)=>{
        ...
        legend.onAdd = () => {
          ...
          let labels = [];
          labels.push("<div>Niveles de Riesgo: Pasto</div>");
